...
void input_parameter() {
	...
	else if (dataset_id == 2) {	// s3

		delta_min = 9000;
		local_density_min = 10;
	}
	else if (dataset_id == 3) {	// synthetic-2
	...
	else if (dataset_id == 5) {	// s4

		delta_min = 100000;
		local_density_min = 1;
	}
	else if (dataset_id == 6) {	// s2

		delta_min = 100000;
		local_density_min = 4;
	}
	else if (dataset_id == 7) {

		delta_min = 4000;
		local_density_min = 50;
	}
	else if (dataset_id == 8) {

		delta_min = 10000;
		local_density_min = 100;
	}
	else if (dataset_id == 9) {

		delta_min = 6500;
		local_density_min = 100;
	}
	else if (dataset_id == 10) {

		delta_min = 20000;
		local_density_min = 100;
	}
	else if (dataset_id == 11) {

		delta_min = 25000;
		local_density_min = 100;
	}
	else if (dataset_id == 12) {

		delta_min = 25000;
		local_density_min = 200;
	}
	else if (dataset_id == 13) {

		delta_min = 1000000;
		local_density_min = 10;
	}
	else if (dataset_id == 14) {

		delta_min = 6000;
		local_density_min = 15;
//		cell_pts_size_min = 5;
	}
	else if (dataset_id == 15) {
...
void input_data() {
	...
	if (dataset_id == 0) {
		...
		std::string f_name = "dataset/synthetic.txt";
...
void input_label() {
	...
	std::string str = "../Ex-DPC/" + directory_comp() + "Label/label_id(" + std::to_string(dataset_id) + ")_sampling_rate(" + std::to_string(sampling_rate) + ")_cutoff(" + std::to_string(cutoff) + ").txt";
...
void output_coord_label() {
	...
	for (unsigned int i = 0; i < cluster_centers.size(); ++i) {
		...
		std::string f_name = "result/" + directory_comp() + "/Label/approx-label-" + std::to_string(it->first) + "_id(" + std::to_string(dataset_id) + ")_cutoff(" + std::to_string(cutoff) + ").txt";
		std::ofstream file;
...
void output_cpu_time() {
	...
	std::string f_name = "result/" + directory_comp() + "/cpu_time_data-id(" + std::to_string(dataset_id) + ")_sampling_rate(" + std::to_string(sampling_rate) + ")_cutoff(" + std::to_string(cutoff) + ")_core_no(" + std::to_string(core_no) + ")_approx.csv";
	std::ofstream file;
	file.open(f_name.c_str(), std::ios::out | std::ios::app);
