...
void grid_partition_by_cost(std::vector<sc_pair>& grid, std::vector<std::vector<std::string>>& grid_partition) {
	...
	boost::sort::block_indirect_sort(grid.begin(), grid.end(), std::greater<sc_pair>(), core_no);

	// cost array (each element suggests the tempral cost of each thread)
...
void idx_partition(std::vector<unsigned int>& idx, std::vector<std::vector<pts>>& dataset_subsets, std::vector<std::vector<unsigned int>>& idx_partitioned) {
	...
	// sort
	boost::sort::block_indirect_sort(idx_cost.begin(), idx_cost.end(), std::greater<std::pair<float, unsigned int>>(), core_no);
