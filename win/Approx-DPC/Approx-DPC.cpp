...
void local_desntiy_computation_large() {
	...
	// local-density computaion
	#pragma omp parallel num_threads(core_no)
	{
		#pragma omp for schedule(static)
		for (int i = 0; i < core_no; ++i) {

			// local-density computaion by scan
...
void dependent_point_computation() {
	...
	boost::sort::sample_sort(dataset_pt.begin(), dataset_pt.end(), desc_local_density, core_no);
	boost::sort::sample_sort(point_set.begin(), point_set.end(), desc_local_density_pts, core_no);
...
void computation_label_propagation() {
	...
	// sort by ID
	boost::sort::block_indirect_sort(dataset_pt.begin(), dataset_pt.end(), asc_id, core_no);
...
int main() {
	...
	// local density computation for large cells
	local_desntiy_computation_large();
