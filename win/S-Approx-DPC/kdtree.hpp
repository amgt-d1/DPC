	...
	*/
	...
	public:
		...
		*/
		...
		void radiusSearch(PointT& query, float radius, std::vector<int>& result, float &distance_to_nn) const
		{
			radiusSearchRecursive(query, root_, result, radius, distance_to_nn);
		}
	...
	private:
		...
		*/
		...
		/** @brief Searches neighbors within radius.
		*/
		void radiusSearchRecursive(const PointT& query, const Node* node, std::vector<int>& indices, const double radius) const {

			if (node == nullptr)	return;

			const PointT& train = points_[node->idx];

			if (query.id != train.id) {
		...
		void radiusSearchRecursive(const PointT& query, const Node* node, std::vector<int>& result, const float radius, float& distance_to_nn) const {
			...
			if (query.id != train.id) {
				...
				if (dist < radius) {
					result.push_back(train.id);
					if (dist < distance_to_nn) distance_to_nn = dist;
				}
			}

			const int axis = node->axis;
			const int dir = query[axis] < train[axis] ? 0 : 1;
			radiusSearchRecursive(query, node->next[dir], result, radius, distance_to_nn);

			const double diff = fabs(query[axis] - train[axis]);
			if (diff < radius) radiusSearchRecursive(query, node->next[!dir], result, radius, distance_to_nn);
		}
