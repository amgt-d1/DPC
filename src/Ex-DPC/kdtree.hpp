	...
	*/
	...
	public:
		...
		*/
		...
		std::vector<int> radiusSearch(PointT& query, double radius, unsigned int& access) const
		{
			std::vector<int> indices;
			radiusSearchRecursive(query, root_, indices, radius, access);
	...
	private:
		...
		*/
		...
		void radiusSearchRecursive(const PointT& query, const Node* node, std::vector<int>& indices, const double radius, unsigned int& access) const {
			...
			const int axis = node->axis;
			const int dir = query[axis] < train[axis] ? 0 : 1;
			radiusSearchRecursive(query, node->next[dir], indices, radius, access);

			const double diff = fabs(query[axis] - train[axis]);
			if (diff < radius) radiusSearchRecursive(query, node->next[!dir], indices, radius, access);
		}
